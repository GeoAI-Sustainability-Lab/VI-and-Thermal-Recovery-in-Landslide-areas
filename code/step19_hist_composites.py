"""Step 19: historical summer composites 2013-2019 on the same 30 m grid.

Extends the epoch stack backwards so the chronosequence becomes a genuine
longitudinal series (each patch observed up to 14 summers) and so that
pre-event epochs exist for cohorts back to 2014.

Sensor consistency is deliberate: Landsat 8 ONLY, exactly the platform and
thermal instrument (TIRS, 100 m native, ST_B10) used for the 2020-2026
epochs. Landsat 5 TM (120 m) and Landsat 7 ETM+ (60 m, SLC-off) are excluded
because a change of thermal footprint across the record would attenuate the
patch-control contrast for small patches in the early epochs and bias the
recovery constant upwards.

Memory-lean version of step01: no sum-of-squares accumulator, no .npz dump;
each summer is composed and written straight to GeoTIFF, then the buffers are
released. Run one summer per process (SAS tokens expire in long processes).

Usage: python3 step19_hist_composites.py 2013 [2014 ...]
(2020 is also rebuilt here: its first build lost 28/46 scenes to token expiry.)
"""
import os as _os
...
def process_item(it_raw, lst_acc, ndvi_acc):
    ...
    st = warp_resigned(it, "lwir11", sub, Resampling.bilinear)
...
def run_summer(year):
    ...
    for band in ("lst", "ndvi"):
        ...
        if band == "lst":
            ...
            ndvi_acc = None
        del acc, cnt, mean
    json.dump(meta, open(f"{OUT}/acc/{tag}_items.json", "w"))
